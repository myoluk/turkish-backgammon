...
var ValidMoves = function(){
  ...
  function setValidMoves(vm) {
    // sırası gelen oyuncunun kirigi varsa
    ...
    else {   // sırası gelen oyuncunun kirigi yoksa
      for (var i=1; i<=24; i++) {
...
var Board = {
  ...
  reset: function(){
    this.p24 = [2, 0];  // 0, siyah
    this.p23 = [0, -1]; // -1, lokasyon bos
    this.p22 = [0, -1];
    this.p21 = [0, -1];
    this.p20 = [0, -1];
    this.p19 = [5, 1];  // 1, beyaz
    this.p18 = [0, -1];
    this.p17 = [3, 1];
    this.p16 = [0, -1];
    this.p15 = [0, -1];
...
function init() {
  ...
  document.getElementById("statusInfo").innerHTML = "Tavla yüklendi.";
}
...
function drawBoard(brd) {
  ...
  var w = brd.width;
  var h = w*3/4;
  var p = 0.02; // tahta hem dikey hem de yatay olarak 50 adima dilimlenir 
  canvas.width = w;
  canvas.height = h;
  drawEmptyBoard();
  // 24 yap
  ...
  function Button(x, y, width, height, text){
    this.x = x;  //x,y butonun merkezini belirtir
    this.y = y;
    this.width = width;
    this.height = height;
    this.text = text;
  ...
  function drawEmptyBoard() {
    ...
    ctx.fillRect(w*p, h*p*2, w*p*3, h*p*20); //sol-ust tutucu
    ctx.fillRect(w*p, h*p*28, w*p*3, h*p*20); //sol-alt tutucu
    ctx.fillRect(w*p*46, h*p*2, w*p*3, h*p*20); //sag-ust tutucu
    ctx.fillRect(w*p*46, h*p*28, w*p*3, h*p*20); //sag-alt tutucu
    ctx.fillRect(w*p*5, h*p*2, w*p*18, h*p*46); //sol tahta
    ctx.fillRect(w*p*27, h*p*2, w*p*18, h*p*46); //sag tahta
    // 24 ucgen ciz
    for (var i = 0; i <= 5; i++) {
      drawTriangle(5 + i*3, 2, 3, 20, i%2); //sol-ust ucgenler
      drawTriangle(5 + i*3, 48, 3, 20, (i+1)%2); //sol-alt ucgenler
      drawTriangle(27 + i*3, 2, 3, 20, i%2); //sag-ust ucgenler
      drawTriangle(27 + i*3, 48, 3, 20, (i+1)%2); //sag-alt ucgenler
    }
    ctx.restore();
  ...
  function drawPoint(j, point) {
    ...
    if (point[0] > 5) {  // sayilari 5'ten fazla ise son kontrol edilene cek
      ctx.save();
  ...
  function drawChecker(x, y, col, hl) {
    ...
    function addShadow() {
      ctx.shadowColor = "#444"; // renk
      ctx.shadowBlur = 5; // bulaniklik
      ctx.shadowOffsetX = r * 0.2; // yatay uzaklik
      ctx.shadowOffsetY = r * 0.2; // dikey uzaklik
    }
  }
  ...
  function drawCube(cb) {
    ...
    cb.x = w*p*2.5;  // x,y merkezi belirtir
    cb.y = h*p*25;
    cb.angle = 0;
    cb.size = w*p*3*0.95;
  ...
  function drawRest() {
    ...
    function drawRollButton() {
      rollButton = new Button(0, h*p*25, w*p*8, h*p*3, "ZAR AT");
      rollButton.x = brd.onMove == "black" ? w*p*36 : w*p*14;
      drawButton(rollButton);
    }
    
    function drawStartButton(){
      startButton = new Button(w*p*25, h*p*25, w*p*10, h*p*4, "BAŞLAT");
    ...
    function drawDices() {
      ...
      function drawDice(ctx, x, y, size, value, diceColor, dotColor) {
        /*  Bu fonksiyon bir zar cizer.
        ctx cerceve baglamidir
        x, y zarin ust kosesinin koordinatlaridir
        size zar boyutunun piksel cinsinden uzunlugudur
        value zarin 1-6 arasinda degeridir 
        diceColor ve DotColor zar govdesinin ve uzerindeki noktalarin renkleridir
        roundRect fonksiyonun bir parcasi degildir fakat kullanilacaktir  */
        dots = [];
        ctx.save();
        ...
        else console.log("Zar değeri 1-6 arasında olmalı");
...
function handleMouseclick(evt) {
  ...
  if (Board.status == "start" && mousePos.point == "startButton") {
    ...
    Board.moves.sort();
    if (Board.onMove == 'black'){
      document.getElementById("statusInfo").innerHTML = "Siyah oyuna başladı.";
    }
    else {
      document.getElementById("statusInfo").innerHTML = "Beyaz oyuna başladı.";
    }
  ...
  if (Board.status == "roll") {
    if (mousePos.point == "rollButton") {
      ...
      Board.moves.sort();
      document.getElementById("statusInfo").innerHTML = "Gelen zar: " + Dices.diceValueLeft + " " + Dices.diceValueRight;
      Board.status = "move";
      drawBoard(Board);
      ...
      if (valids.numOfValidMoves() == 0) setTimeout(function(){
        ...
        Board.status = "roll";
        Board.moves = [];
        document.getElementById("statusInfo").innerHTML = "Kullanılabilir hamle yok. El değiş."
        drawBoard(Board);
      }, 1200);
    }
      ...
      rollButton.onScreen = false;
      document.getElementById("statusInfo").innerHTML = Board.onMove.capitalizeFirstLetter() + " katlandı. Kabul ediyor musun?";
      Board.onMove = Board.onMove == "black" ? "white" : "black";
  ...
  if (Board.status == "move") {
    ...
    if (mousePos.point.charAt(0) == "p" || mousePos.point.charAt(0) == "b") {
      if (Board[mousePos.point][1] != (Board.onMove == "black" ? 0 : 1)) {
        document.getElementById("statusInfo").innerHTML = "Bu senin pulun değil!";
        return;
      }
      if (Board.moves.length == 0) {
        document.getElementById("statusInfo").innerHTML = "Zaten hamle yaptın. Zarı al.";
        return;
      }
      start = mousePos.point;
      if (!valids.hasOwnProperty(start) || valids[start].length == 0) {
        document.getElementById("statusInfo").innerHTML = "Geçerli hamle yok.";
        return;
      } 
      ...
      // zari al
      if (valids.numOfValidMoves() == 0) {
        // oyunun sonunu algila
        if (Board["off"+Board.onMove.capitalizeFirstLetter()][0] == 15) {
          document.getElementById("statusInfo").innerHTML = Board.onMove.capitalizeFirstLetter() + " oyunu kazandı. Yenisi için başlata basın.";
          Dices.onScreen = false;
          Board.status = "start";
          drawBoard(Board);
          return;
        ...
        Board.moves = [];
        Board.playedMoves = [];
        document.getElementById("statusInfo").innerHTML = "Yeni el."
        drawBoard(Board);
      }
    }
  }
  ...
  if (Board.status == "accept") {
    if (mousePos.point == "takeButton") {
      takeButton.onScreen = false;
      passButton.onScreen = false;
      document.getElementById("statusInfo").innerHTML = Board.onMove.capitalizeFirstLetter() + " küpü aldı. Yeni el.";
      ...
      Board.status = "start";
      document.getElementById("statusInfo").innerHTML = winner + " oyunu kazandı. Yenisi için başlata basın.";
      drawBoard(Board);
    }
  }
}
...
function move(start, target) {
  ...
  // hedef, hareket halindeki oyuncuya aitse
  else if (Board[target][1] == Board[start][1]) Board[target][0]++;
  // oyuncu hedef noktaya isabet ederse
  else {
  ...
  document.getElementById("statusInfo").innerHTML = "Hamle: " + start + "/" + target;
}

function getMousePos(canvas, evt) {
  /*  Bu fonksiyon farenin konumunu alir ve obje olarak dondurur:
  {
  x: tahta uzerindeki farenin yatay konumu,
  y: tahta uzerindeki farenin yatay konumu,
  xp: tahta uzerindeki farenin yatay kare konumu (0-50),
  yp: tahta uzerindeki farenin dikey kare konumu (0-50),
  point: tahtanin string olarak alani
  }
  */
